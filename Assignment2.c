...
int **gameoflife(int **a, int N, int maxgen, int threads)
{
...
int main(int argc, char **argv) 
{
    ...
    if (argc != 4) {
      printf("Three arguments needed. Usage: %s <N>; Generations; thread count\n", argv[0]);
      exit(-1);
    }
    ...
    test = gameoflife(prev, N, maxgen, threads);
    printf("Final\n");
    printgame(test, N, N);
    //printarray(test, N, N);
    endtime = gettime();
...
#endif
    gameoflife(prev, N, maxgen, threads);
